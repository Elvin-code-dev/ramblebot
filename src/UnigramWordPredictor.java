...
public class UnigramWordPredictor implements WordPredictor {
  ...
  public UnigramWordPredictor(Tokenizer tokenizer) {
    ...
    this.neighborMap = new HashMap<>(); // didnt know why it wasent working but firgured out that we should start a
                                        // empty map
  }
  ...
  public void train(Scanner scanner) {
    ...
    // want to start fresh each time we train
    neighborMap.clear();

    // here i link each word to the word that comes right after it
    // keep duplicates so we keep thereal frequencies
    for (int i = 0; i < trainingWords.size() - 1; i++) {
      String curr = trainingWords.get(i);
      String next = trainingWords.get(i + 1);

      // makeing a list if this word is new
      List<String> list = neighborMap.get(curr);
      if (list == null) {
        list = new ArrayList<>();
        neighborMap.put(curr, list);
      }

      // here i add the next word
      list.add(next);
    }
